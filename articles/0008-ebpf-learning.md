...
# 結論
eBPF の良さは、ネットワーク、セキュリティ、可観測性などカバレッジの広さにあると思います。ただし、カーネルに関する専門性が必要になるため導入するハードルの高さもあります。セキュリティに関して Tetragon はポリシーの適用のみで利用でき、特定のユースケースに柔軟に対応できるため eBPF を初めて導入する際や各サーバにセキュリティのレイヤを設ける場合に活用できると思います。
