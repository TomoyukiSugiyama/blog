...
BPF ( BSD Packet Filter ) が，効率的なパケットフィルタリング手法として提案

- ネットワークパケットを通過させるか、拒絶されるかを決定するためのプログラム
- フィルタを動かすための擬似的な機械

1997 年

BPF ( Berkeley Packet Filter ) が、Linux (カーネルバージョン : 2.1.75 ) に初めて導入

- tcpdump の中で、パケットのキャプチャに利用

2012 年

seccomp-bpf が、Linux (カーネルバージョン : 3.5) に導入

- ユーザ空間のアプリケーションからのシステムコール呼び出し許可、禁止を決定
...
- アプリケーション ・・・ プロファイリング、モニタリング、トレーシング
- カーネル・・・監視、セキュリティ、ネットワーキング、ロードバランシング
